...
export function PurchaseFormFields({
	...
	const [exerId, setExerId] = useState(initialPurchase?.exerciceId ?? "");
	...
	const [paidByCltAmount, setPaidByCltAmount] =
		useState(initialPurchase?.paidByCltAmount ?? 0);
	const [paidByThirdPartyAmount, setPaidByThirdPartyAmount] =
		useState(initialPurchase?.paidByThirdPartyAmount ?? 0);
...
useEffect(() => {
  ...
  onChange,
	...
	//console.log("Achats, sociétés ", options.societes);
	...
	return (
		<>
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-x-10 gap-y-0">
				...
				<PurchaseAmountHtField value={amountHt} onChange={setAmountHt} error={amountHtError} />
				<PurchaseAmountTaxField value={amountTax} onChange={setAmountTax} error={amountTaxError} />
				<PurchaseAmountTtcField value={amountTtc} onChange={setAmountTtc} error={amountTtcError} />
